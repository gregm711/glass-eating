...
import { TokenProgramAccount, TokenAccount, MintAccount, tryGetMint, getOwnedTokenAccounts, parseMintAccountData, parseTokenAccountData, getMultipleAccountInfoChunked } from '../utils/tokens'

import { getGovernance, getGovernanceAccount, getGovernanceAccounts, getGovernanceProgramVersion, Governance, GovernanceAccountType, GOVERNANCE_CHAT_PROGRAM_ID, Proposal, ProposalTransaction, Realm, SignatoryRecord, TokenOwnerRecord, VoteRecord } from '@solana/spl-governance'
...
import { getTokenOwnerRecordsForRealmMintMapByOwner, getVoteRecordsByProposalMapByVoter, getVoteRecordsByVoterMapByProposal } from '@models/api'
...
async function mapFromPromisedEntries(xs: any, mapFn: (kv: [string, any]) => Promise<[string, any]>) {
...
const useWalletStore = create<WalletStore>((set, get) => ({
	...
	actions: {
		async fetchRealmBySymbol(cluster: string, symbol: string) {
			...
			if (connection.cluster !== newConnection.cluster || connection.endpoint !== newConnection.endpoint) {
				set((s) => {
			...
			if (!realmId) {
				...
				const realmAccountInfo = await connection.current.getAccountInfo(realmId)
				programId = realmAccountInfo?.owner
			}
			if (realmId && programId) {
				const programVersion = await getGovernanceProgramVersion(connection.current, programId!)
				set((s) => {
		...
		async fetchWalletTokenAccounts() {
			...
			if (connected && walletOwner) {
				const ownedTokenAccounts = await getOwnedTokenAccounts(connection, walletOwner)
				console.log(
		...
		async fetchOwnVoteRecords() {
			...
			if (connected && walletOwner && programId) {
				const ownVoteRecordsByProposal = await getVoteRecordsByVoterMapByProposal(connection, programId, walletOwner)
		...
		async fetchRealm(programId: PublicKey, realmId: PublicKey) {
			...
			const mintsArray = (await Promise.all([realm?.account.communityMint ? tryGetMint(connection, realm.account.communityMint) : undefined, realm?.account.config?.councilMint ? tryGetMint(connection, realm.account.config.councilMint) : undefined])).filter(Boolean)

			set((s) => {
				s.selectedRealm.mints = Object.fromEntries(mintsArray.map((m) => [m!.publicKey.toBase58(), m!.account]))
			})
			...
			const realmCouncilMint = realmCouncilMintPk && realmMints[realmCouncilMintPk.toBase58()]
			const [governances, tokenRecords, councilTokenOwnerRecords] = await Promise.all([getGovernanceAccounts(connection, programId, Governance, [pubkeyFilter(1, realmId)!]), getTokenOwnerRecordsForRealmMintMapByOwner(connection, programId, realmId, realmMintPk), getTokenOwnerRecordsForRealmMintMapByOwner(connection, programId, realmId, realmCouncilMintPk)])
			...
			const proposalsByGovernance = await Promise.all(governances.map((g) => getGovernanceAccounts(connection, programId, Proposal, [pubkeyFilter(1, g.pubkey)!])))

			const proposals = accountsToPubkeyMap(proposalsByGovernance.flatMap((p) => p).filter((p) => !HIDDEN_PROPOSALS.has(p.pubkey.toBase58())))
			...
			const proposalDescriptions = await mapFromPromisedEntries(proposals, async ([k, v]: [string, ProgramAccount<Proposal>]) => {
				return [k, await resolveProposalDescription(v.account.descriptionLink)]
			})
		...
		async fetchProposal(proposalPk: string) {
			...
			const proposal = await getGovernanceAccount<Proposal>(connection, proposalPubKey, Proposal)

			const proposalMint = realmMints[proposal.account.governingTokenMint.toBase58()]

			const programId = proposal.owner

			const [description, governance, instructions, voteRecordsByVoter, signatories, chatMessages, proposalOwner] = await Promise.all([resolveProposalDescription(proposal.account.descriptionLink), getGovernanceAccount(connection, proposal.account.governance, Governance), getGovernanceAccounts(connection, programId, ProposalTransaction, [pubkeyFilter(1, proposalPubKey)!]), getVoteRecordsByProposalMapByVoter(connection, programId, proposalPubKey), getGovernanceAccounts(connection, programId, SignatoryRecord, [pubkeyFilter(1, proposalPubKey)!]), getGovernanceChatMessages(connection, GOVERNANCE_CHAT_PROGRAM_ID, proposalPubKey), getGovernanceAccount(connection, proposal.account.tokenOwnerRecord, TokenOwnerRecord)])

			const realm = await getGovernanceAccount(connection, governance.account.realm, Realm)

			const tokenType = realm.account.communityMint.equals(proposal.account.governingTokenMint) ? GoverningTokenType.Community : GoverningTokenType.Council
		...
		async fetchChatMessages(proposalPubKey: PublicKey) {
			...
			const chatMessages = await getGovernanceChatMessages(connection, GOVERNANCE_CHAT_PROGRAM_ID, proposalPubKey)
		...
		async fetchTokenAccountAndMintsForSelectedRealmGovernances() {
			const { fetchTokenAccountsForSelectedRealmGovernances, fetchMintsForTokenAccounts } = get().actions
			await fetchTokenAccountsForSelectedRealmGovernances()
			fetchMintsForTokenAccounts(get().selectedRealm.tokenAccounts)
		},
		async fetchMintsForTokenAccounts(tokenAccounts: TokenProgramAccount<AccountInfo>[]) {
			const set = get().set
			const connection = get().connection.current
			...
			tokenAccountsMintInfo.forEach((tokenAccountMintInfo, index) => {
				...
				if (!tokenAccountMintInfo) {
					throw new Error(`Missing tokenAccountMintInfo: ${publicKey.toBase58()}`)
				}
		...
		async fetchTokenAccountsForSelectedRealmGovernances() {
			const set = get().set
			const selectedRealmGovernances = Object.values(get().selectedRealm.governances)
			const connection = get().connection.current
			const tokenAccounts: TokenProgramAccount<AccountInfo>[] = []
			const tokenGovernances = selectedRealmGovernances.filter((gov) => gov.account?.accountType === GovernanceAccountType.TokenGovernanceV1 || gov.account?.accountType === GovernanceAccountType.TokenGovernanceV2)
			...
			tokenAccountsInfo.forEach((tokenAccountInfo, index) => {
				...
				const ProgramAccountInfo = parseTokenAccountData(publicKey, data) as AccountInfo
				tokenAccounts.push({
					publicKey: publicKey,
		...
		async fetchVoteRecords(proposal: ProgramAccount<Proposal>) {
			...
			const programId = proposal.owner
			const voteRecordsByVoter = await getVoteRecordsByProposalMapByVoter(connection, programId, proposal.pubkey)
